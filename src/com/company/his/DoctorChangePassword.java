...
public class DoctorChangePassword extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.doctorchangepassword);
		...
		change.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				try{
			     	...
			     	Cursor c=MainActivity.db.rawQuery("SELECT password FROM doctor WHERE doctorID='"+ DoctorLog.userl +"'" , null);
		        	c.moveToFirst();
		        	while (!c.isAfterLast()){
		        		  // preparing list data
		        	     // prepareListData(c.getString(0));
		        	if(DoctorLog.userl.toString().equals(c.getString(0))){

				if(c.getString(1).equals(oldPassword)){
					MainActivity.db.beginTransaction();
					MainActivity.db.execSQL("UPDATE doctor SET password='"+ changepassword +"' WHERE doctorID= '"+DoctorLog.userl+"'");
					...
					Toast.makeText(getApplicationContext(), "old assword wrong",
		        			   Toast.LENGTH_LONG).show();
				}
		        	      c.moveToNext();
		        	}
